...
class JulianPeriod {
  ...
  /**
   * Convert a Julian Day Number (JD) to a Gregorian date and time.
   *
   * @param {number} jd Julian Day Number (JD)
   * @returns {GregorianDateTime} Gregorian date and time
   * @throws {TypeError} jd must be a finite number
   */
  jdToGregorian(jd: number): GregorianDateTime {
    ...
    const dim = daysInMonth(year, month, isGregorian);
    if (day > dim) {
      day = 1;
      month += 1;
      if (month > 12) {
